...
def readTrajectory(filename):
	...
	for line in traj_file:
		...
		if(reading_filament):
			...
			for i in range(0, len(FS.coords) - 1):
				CS = CylinderSnapshot()
...
def readTrajectories(fileNames, fileDirectory='', runMin=0, runMax=10):
	...
	if (fileDirectory != ''):
		...
		for trajNum in range(runMin, runMax):
...
def combineTwoHistograms(histSnapshot1, histSnapshot2):
	...
	#combine hist values
	for bins, freq in histSnapshot1.hist.items():
		H.hist[bins] = freq

	for bins, freq in histSnapshot2.hist.items():
		H.hist[bins] += freq
...
def combineAllHistograms(histSnapshotList):
	...
	for i in range(2, len(histSnapshotList), 1):
...
def radiusOfGyration(SnapshotList, snapshot=1):
	...
	for cylinderID, CS in Snapshot.cylinders.items():
...
def contractileVelocity(SnapshotList):
	...
	for SnapshotIndex in range(0, len(SnapshotList) - 1):
		...
		#go through filaments
		for filamentID, FS in SnapshotI.filaments.items():
			for coord in FS.coords:
		...
		numCylinders = 0
		#Loop through cylinders
		for cylinderIDA, CSA in SnapshotF.cylinders.items():
...
def meanSquareDisplacement(SnapshotList):
	...
	for SnapshotIndex in range(1, len(SnapshotList), 1):
		...
		#Loop through cylinders
		for filamentIDA, FSA in SnapshotF.filaments.items():
...
def density(Snapshot, grid, compartment):
	...
	for index, coordinate in compartmentMap.items():
...
#Below are functions added by Chengxuan, including graph theory parameters


#Calculate average mean displacement of filaments 
#returns a list of time and md pairs
def meanDisplacement(SnapshotList):
	...
	for SnapshotIndex in range(1, len(SnapshotList), 1):
		...
		SnapshotI = SnapshotList[SnapshotIndex-1]
		SnapshotF = SnapshotList[SnapshotIndex]
		...
		for filamentIDA, FSA in SnapshotF.filaments.items():
			...
			#calc mean displacement (in nm)
			sd = ((comB - comA).dot(comB - comA))**0.5
            #sd = (comB - comA).dot(comB - comA) * 0.001 * 0.001
		...
		md = np.mean(sds)
		mds.append((SnapshotF.time, md))

	#return msds
	return mds

#Calculate the shape parameter over all cylinders
#returns a time and S pair for the chosen snapshot
def shapeparameter(SnapshotList, snapshot=1):
    Snapshot = SnapshotList[snapshot]
    
    #create the inertial tensor
    ITensor = np.zeros(shape=(3,3))
    #Initialization of beads
    beadCount = 0
    
    #loop through filament beads
    for filamentID1, FS1 in Snapshot.filaments.items():
        beadCount+=len(FS1.coords)
        for filamentID2, FS2 in Snapshot.filaments.items():
            for i in range(0, len(FS1.coords)): 
                for j in range(0, len(FS2.coords)):
                    ITensor[0,0] += (FS1.coords[i,0]-FS2.coords[j,0])*(FS1.coords[i,0]-FS2.coords[j,0])
                    ITensor[0,1] += (FS1.coords[i,0]-FS2.coords[j,0])*(FS1.coords[i,1]-FS2.coords[j,1])
                    ITensor[0,2] += (FS1.coords[i,0]-FS2.coords[j,0])*(FS1.coords[i,2]-FS2.coords[j,2])
                    ITensor[1,0] += (FS1.coords[i,1]-FS2.coords[j,1])*(FS1.coords[i,0]-FS2.coords[j,0])
                    ITensor[1,1] += (FS1.coords[i,1]-FS2.coords[j,1])*(FS1.coords[i,1]-FS2.coords[j,1])
                    ITensor[1,2] += (FS1.coords[i,1]-FS2.coords[j,1])*(FS1.coords[i,2]-FS2.coords[j,2])
                    ITensor[2,0] += (FS1.coords[i,2]-FS2.coords[j,2])*(FS1.coords[i,0]-FS2.coords[j,0])
                    ITensor[2,1] += (FS1.coords[i,2]-FS2.coords[j,2])*(FS1.coords[i,1]-FS2.coords[j,1])
                    ITensor[2,2] += (FS1.coords[i,2]-FS2.coords[j,2])*(FS1.coords[i,2]-FS2.coords[j,2])
    ...
    #normalize and get inertial tensor
    ITensor /= 2 * beadCount ** 2
    
    #get the eigenvalues of the inertial tensor
    lamda = np.linalg.eigvals(ITensor)
    #get the average eigenvalue of the inertial tensor
    averagelamda = np.average(lamda)
    tensortrace = np.trace(ITensor)
    cubedtrace = tensortrace ** 3
    shapeparameter = 27 * ((lamda[0]-averagelamda)*(lamda[1]-averagelamda)*(lamda[2]-averagelamda)) / cubedtrace
    
    return (Snapshot.time, shapeparameter)        

#not coarse-grained graph
def PairwiseDistance(SnapshotList,snapshot):
    Snapshot = SnapshotList[snapshot]
    Fcoordslist=[]
    for filamentid, FS in Snapshot.filaments.items():
        for x in FS.coords:
    ...
    pairwisedist=scipy.spatial.distance.squareform(scipy.spatial.distance.pdist(Fcoordsarray, 'euclidean'))
    Gmatrix=nx.from_numpy_matrix(pairwisedist<20)
    return (Snapshot.time, pairwisedist,Gmatrix)

def community(SnapshotList,snapshot):
    Snapshot = SnapshotList[snapshot]
    PWDist=PairwiseDistance(SnapshotList,snapshot)
    G=PWDist[2]
    community = greedy_modularity_communities(G)
    return (Snapshot.time,community)

def communitylist(SnapshotList):
    communitylist = []
    for x in range(1,len(SnapshotList),1):
        communitylist.append(community(SnapshotList,x))
    return (communitylist)
    
def average_clustering(SnapshotList,snapshot):
    Snapshot = SnapshotList[snapshot]
    PWDist=PairwiseDistance(SnapshotList,snapshot)
    G=PWDist[2]
    average_clustering = nx.average_clustering(G)
    return (Snapshot.time,average_clustering)   

def average_clusteringlist(SnapshotList):
    average_clusteringlist = []
    for x in range(1,len(SnapshotList),1):
        average_clusteringlist.append(average_clustering(SnapshotList,x))
    return (average_clusteringlist)
 
#print (average_clusteringlist(snapshotlist1))  

def clique_number(SnapshotList,snapshot):
    Snapshot = SnapshotList[snapshot]
    PWDist=PairwiseDistance(SnapshotList,snapshot)
    G=PWDist[2]
    clique_number = nx.algorithms.clique.graph_clique_number(G)
    return (Snapshot.time,clique_number)  

def clique_numberlist(SnapshotList):
    clique_numberlist = []
    for x in range(1,len(SnapshotList),1):
        clique_numberlist.append(clique_number(SnapshotList,x))
    return (clique_numberlist)

def closeness_cent(SnapshotList,snapshot):
    Snapshot = SnapshotList[snapshot]
    PWDist=PairwiseDistance(SnapshotList,snapshot)
    G=PWDist[2]
    closeness_cent = np.mean(list(nx.closeness_centrality(G).values()))
    return (Snapshot.time,closeness_cent)  

def closeness_centlist(SnapshotList):
    closeness_centlist = []
    for x in range(1,len(SnapshotList),1):
        closeness_centlist.append(closeness_cent(SnapshotList,x))
    return (closeness_centlist)

def mean_betweenness_cent(SnapshotList,snapshot):
    Snapshot = SnapshotList[snapshot]
    PWDist=PairwiseDistance(SnapshotList,snapshot)
    G=PWDist[2]
    mean_betweenness_cent = np.mean(list(nx.betweenness_centrality(G).values()))
    return (Snapshot.time,mean_betweenness_cent)  

def mean_betweenness_centlist(SnapshotList):
    mean_betweenness_centlist = []
    for x in range(1,len(SnapshotList),1):
        mean_betweenness_centlist.append(mean_betweenness_cent(SnapshotList,x))
    return (mean_betweenness_centlist)

def max_betweenness_cent(SnapshotList,snapshot):
    Snapshot = SnapshotList[snapshot]
    PWDist=PairwiseDistance(SnapshotList,snapshot)
    G=PWDist[2]
    max_betweenness_cent = np.max(list(nx.betweenness_centrality(G).values()))
    return (Snapshot.time,max_betweenness_cent)  

def max_betweenness_centlist(SnapshotList):
    max_betweenness_centlist = []
    for x in range(1,len(SnapshotList),1):
        max_betweenness_centlist.append(max_betweenness_cent(SnapshotList,x))
    return (max_betweenness_centlist)

def eigen_cent(SnapshotList,snapshot):
    Snapshot = SnapshotList[snapshot]
    PWDist=PairwiseDistance(SnapshotList,snapshot)
    G=PWDist[2]
    try:
        eigen_cent = np.mean(list(nx.eigenvector_centrality(G).values()))
    except:
        eigen_cent = np.nan
    return (Snapshot.time,eigen_cent)  

def eigen_centlist(SnapshotList):
    eigen_centlist = []
    for x in range(1,len(SnapshotList),1):
        eigen_centlist.append(eigen_cent(SnapshotList,x))
    return (eigen_centlist)

def assort(SnapshotList,snapshot):
    Snapshot = SnapshotList[snapshot]
    PWDist=PairwiseDistance(SnapshotList,snapshot)
    G=PWDist[2]
    assort = nx.degree_assortativity_coefficient(G)
    return (Snapshot.time,assort)  

def assortlist(SnapshotList):
    assortlist = []
    for x in range(1,len(SnapshotList),1):
        assortlist.append(assort(SnapshotList,x))
    return (assortlist)

def dens(SnapshotList,snapshot):
    Snapshot = SnapshotList[snapshot]
    PWDist=PairwiseDistance(SnapshotList,snapshot)
    G=PWDist[2]
    dens = nx.density(G)
    return (Snapshot.time,dens)  

def denslist(SnapshotList):
    denslist = []
    for x in range(1,len(SnapshotList),1):
        denslist.append(dens(SnapshotList,x))
    return (denslist)

def average_degree_connect(SnapshotList,snapshot):
    Snapshot = SnapshotList[snapshot]
    PWDist=PairwiseDistance(SnapshotList,snapshot)
    G=PWDist[2]
    average_degree_connect  = nx.average_degree_connectivity(G)
    return (Snapshot.time,average_degree_connect)  

def average_degree_connectlist(SnapshotList):
    average_degree_connectlist = []
    for x in range(1,len(SnapshotList),1):
        average_degree_connectlist.append(average_degree_connect(SnapshotList,x))
    return (average_degree_connectlist)

def average_neighbor_degree(SnapshotList,snapshot):
    Snapshot = SnapshotList[snapshot]
    PWDist=PairwiseDistance(SnapshotList,snapshot)
    G=PWDist[2]
    average_neighbor_degree = nx.average_neighbor_degree(G)
    return (Snapshot.time,average_neighbor_degree)  

def average_neighbor_degreelist(SnapshotList):
    average_neighbor_degreelist = []
    for x in range(1,len(SnapshotList),1):
        average_neighbor_degreelist.append(average_neighbor_degree(SnapshotList,x))
    return (average_neighbor_degreelist)

def g_radius(SnapshotList,snapshot):
    Snapshot = SnapshotList[snapshot]
    PWDist=PairwiseDistance(SnapshotList,snapshot)
    G=PWDist[2]
    try:
        g_radius = nx.radius(G) # max eccercity 
    except:
        g_radius = np.nan
    return (Snapshot.time,g_radius)  

def g_radiuslist(SnapshotList):
    g_radiuslist = []
    for x in range(1,len(SnapshotList),1):
        g_radiuslist.append(g_radius(SnapshotList,x))
    return (g_radiuslist)

def g_diameter(SnapshotList,snapshot):
    Snapshot = SnapshotList[snapshot]
    PWDist=PairwiseDistance(SnapshotList,snapshot)
    G=PWDist[2]
    try:
        g_diameter = nx.diameter(G) # min eccercity 
    except:
        g_diameter = np.nan
    return (Snapshot.time,g_diameter)  

def g_diameterlist(SnapshotList):
    g_diameterlist = []
    for x in range(1,len(SnapshotList),1):
        g_diameterlist.append(g_diameter(SnapshotList,x))
    return (g_diameterlist)

def normalized_wiener_index(SnapshotList,snapshot):
    Snapshot = SnapshotList[snapshot]
    PWDist=PairwiseDistance(SnapshotList,snapshot)
    G=PWDist[2]
    try:
        normalized_wiener_index = ((nx.number_of_nodes(G)-1)*nx.number_of_nodes(G)/2)/nx.wiener_index(G)
    except:
        normalized_wiener_index = np.nan
    return (Snapshot.time,normalized_wiener_index)  

def normalized_wiener_indexlist(SnapshotList):
    normalized_wiener_indexlist = []
    for x in range(1,len(SnapshotList),1):
        normalized_wiener_indexlist.append(normalized_wiener_index(SnapshotList,x))
    return (normalized_wiener_indexlist)

def ave_node_connectivity(SnapshotList,snapshot):
    Snapshot = SnapshotList[snapshot]
    PWDist=PairwiseDistance(SnapshotList,snapshot)
    G=PWDist[2]
    ave_node_connectivity = nx.average_node_connectivity(G)
    return (Snapshot.time,ave_node_connectivity)  

def ave_node_connectivitylist(SnapshotList):
    ave_node_connectivitylist = []
    for x in range(1,len(SnapshotList),1):
        ave_node_connectivitylist.append(ave_node_connectivity(SnapshotList,x))
    return (ave_node_connectivitylist)        
...
def calculateRgVsT(fileDirectory=''):
	...
	for snap in range(2, len(FrameLists[0]), 2):
