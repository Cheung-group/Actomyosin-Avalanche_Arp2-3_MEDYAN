...
for line in traj_file:
	...
	if(first_snapshot_line):
		...
		if len(line.split()) == 7:
			...
			F.n_motors, F.n_branchers, F.n_bubbles = list(map(double,line.split()))
		else:
			F.step, F.time, F.n_filaments, F.n_linkers, \
			F.n_motors, F.n_branchers = list(map(double,line.split()))
	...
	if(first_line):
		...
		if(line_split[0] == "FILAMENT") or (line_split[0] == "F"):
			...
			if(len(line_split[1:]) == 5):
				FS.id, FS.type, FS.length, FS.delta_left, FS.delta_right = list(map(int,line_split[1:]))
			else:
				FS.id, FS.length, FS.delta_left, FS.delta_right = list(map(int,line_split[1:]))
		...
		if(line_split[0] == "LINKER") or (line_split[0] == "L"):
			LS = LinkerSnapshot()
			LS.id, LS.type = list(map(int, line_split[1:]))
			first_line = False
			F.linkers[LS.id] = LS
		...
		if(line_split[0] == "MOTOR") or (line_split[0] == "M"):
			MS = MotorSnapshot()
			MS.id, MS.type = list(map(int, line_split[1:-1]))
			first_line = False
			F.motors[MS.id] = MS
		...
		if(line_split[0] == "BRANCHER") or (line_split[0] == "B"):
			BS = BrancherSnapshot()
			BS.id, BS.type = list(map(int, line_split[1:]))
			first_line = False
		...
		if(line_split[0] == "BUBBLE"):
			US = BubbleSnapshot()
			US.id, US.type = list(map(int, line_split[1:]))
			first_line = False
			F.bubbles[US.id] = US
			n_beads_bubble+=1
			reading_bubble=True
...
def show_snapshot(snapshot_number=-1):
	...
	#grid size
	GRIDSIZEMAXX = 10000.0
	GRIDSIZEMINX = 0.0

	GRIDSIZEMAXY = 10000.0
	GRIDSIZEMINY = 0.0

	GRIDSIZEMAXZ = 10000.0
	GRIDSIZEMINZ = 0.0
	...
	BRANCHERCOLOR = (1.0,1.0,0.1)

	local_snapshot=TrajSnapshotList[snapshot_number]
	figw = mlab.figure(1, size=(10000, 10000), bgcolor=(1.0,1.0,1.0))
	mlab.clf()
	...
	if(len(local_snapshot.filaments) != 0):
		...
		if (COLORBYANGLE): 
			for filamentID, FS in local_snapshot.filaments.items():

				#loop through cylinders
				for i in range(1, len(FS.coords[0]), 1):
		...
		if(TRACKENDS):
			...
			gsphere=mlab.pipeline.glyph(plusendsrc, mode="sphere", resolution=24, 
								        scale_mode='scalar', scale_factor=150.0, color=(PLUSENDCOLOR))
			...
			gsphere=mlab.pipeline.glyph(minusendsrc, mode="sphere", resolution=24, 
								        scale_mode='scalar', scale_factor=150.0, color=(MINUSENDCOLOR))
		...
		tube=mlab.pipeline.tube(src, tube_radius=50)
	...
	if(len(local_snapshot.linkers) != 0):
		...
		for i in range(n_beads_linker, n_beads_linker + 2 * n_diffusing_linkers, 2) :
	...
	if(len(local_snapshot.motors) != 0):
		...
		for i in range(n_beads_motor, n_beads_motor + 2 * n_diffusing_motors, 2) :
	...
	if(len(local_snapshot.branchers) != 0):
		...
		src = mlab.pipeline.scalar_scatter(x[0], x[1], x[2])
		gsphere=mlab.pipeline.glyph(src, mode="sphere", resolution=24, 
									scale_factor=15.0, color=BRANCHERCOLOR)
		#tube=mlab.pipeline.tube(src, tube_radius=4)
		#tube.filter.number_of_sides=12
		#surface = mlab.pipeline.surface(tube, color=BRANCHERCOLOR)
