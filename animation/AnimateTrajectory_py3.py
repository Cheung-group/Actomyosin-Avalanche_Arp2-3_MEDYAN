...
def show_snapshot(snapshot_number=-1):
	...
	if(len(local_snapshot.filaments) != 0):
		...
		if(TRACKENDS):
			...
			gsphere=mlab.pipeline.glyph(plusendsrc, mode="sphere", resolution=24, 
								        scale_mode='scalar', scale_factor=15.0, color=(PLUSENDCOLOR))
			...
			gsphere=mlab.pipeline.glyph(minusendsrc, mode="sphere", resolution=24, 
								        scale_mode='scalar', scale_factor=15.0, color=(MINUSENDCOLOR))
	...
	if(saving):
		mlab.savefig(filename=saveFile + "Snapshot" + '%03i'%(snapshot_number) + ".png", size=(400,400))
