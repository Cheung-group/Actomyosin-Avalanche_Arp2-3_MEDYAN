...
for line in traj_file:
    ...
    if(first_snapshot_line):
        ...
        if len(line.split()) == 7:
            ...
            F.n_motors, F.n_branchers, F.n_bubbles = map(float,line.split())
        else:
            F.step, F.time, F.n_filaments, F.n_linkers, \
            F.n_motors, F.n_branchers = map(float,line.split())
...
def show_snapshot(snapshot_number=-1, orientation=(0,235)):
    ...
    mlab.text(0.15, 0.9, time, color=(0.0,0.0,0.0),width=0.3)
    
    global ax    
    ax=mlab.axes(x_axis_visibility=False,y_axis_visibility=False,z_axis_visibility=False,nb_labels=2,extent=[0,1000,0,1000,0,1000])
    ax.axes.font_factor=2
    ...
    if(len(local_snapshot.filaments) != 0):
        ...
        if(len(c) != 0):
            surface = mlab.pipeline.surface(tube, colormap=COLORMAP, vmax = MAXVAL, vmin = MINVAL)
            cb = mlab.colorbar(object=surface, orientation='vertical', 
                                 title=SCALETITLE, label_fmt='%.2f', nb_labels=5)
#            cb = mlab.colorbar(object=surface, orientation='vertical', 
#                                 )
#            cb.data_range = (-2.0,2.0)        
    ...
    if(len(local_snapshot.linkers) != 0):
        ...
        gsphere=mlab.pipeline.glyph(src, mode="sphere", resolution=24, 
                                    scale_mode='none', color=(BRANCHENDCOLOR))
...
#        else:
#            surface = mlab.pipeline.surface(tube, color=BRANCHENDCOLOR)
...
#        src = mlab.pipeline.scalar_scatter(x[0], x[1], x[2])
#        gsphere=mlab.pipeline.glyph(src, mode="sphere", resolution=24, 
#                                    scale_factor=0.1, color=BRANCHENDCOLOR)
